...
public class IntegerSorterTester {
	...
	static Sorter<Entero> enteroSort;
	...
	public static void main(String[] args) { 
		...
		enteroSort = new BubbleSortSorter<Entero>();
	...
	private static Entero[] randomValuesEnteros(int i) {
	...
	private static void testEntero(String msg, Comparator<Entero> cmp) { 
		...
		for (int i=1; i<=20; i += 5) { 
			...
			original = randomValuesEnteros(i);
			showArrayEntero("\n ---Original array of size " + i + " to sort:", original); 
			
			arr = original.clone(); 
		    enteroSort.sort(arr, cmp);
			showArrayEntero(enteroSort.getName() + ": ", arr); 
			
		}
	}

	private static void showArrayEntero(String msg, Entero[] original) {
		System.out.print(msg); 
